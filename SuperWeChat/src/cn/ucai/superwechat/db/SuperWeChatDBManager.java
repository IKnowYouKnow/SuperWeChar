...
public class SuperWeChatDBManager {
    ...
    synchronized public Map<String, User> getAppContactList() {
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.TB_NAME /* + " desc" */, null);
            while (cursor.moveToNext()) {
                User user = new User();
                String username = cursor.getString(cursor.getColumnIndex(UserDao.USER_COLUMN_USERNAEM));
    ...
    synchronized public void deleteAppContact(String username){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if(db.isOpen()){
            db.delete(UserDao.TB_NAME, UserDao.USER_COLUMN_USERNAEM + " = ?", new String[]{username});
        }
    }
    ...
    synchronized public void saveAppContact(User user){
        ...
        values.put(UserDao.USER_COLUMN_AVATAR_LAST_UPDATETIME,user.getMAvatarLastUpdateTime());
        if(db.isOpen()){
            db.replace(UserDao.TB_NAME, null, values);
        }
    }
    ...
    public synchronized Integer saveMessage(InviteMessage message){
        ...
        if(db.isOpen()){
            ...
            values.put(InviteMessgeDao.COLUMN_USER_NICK,message.getNick());
            values.put(InviteMessgeDao.COLUMN_USER_AVATAR,message.getUserAvatar());
    ...
    synchronized public List<InviteMessage> getMessagesList(){
        ...
        if(db.isOpen()){
            ...
            while(cursor.moveToNext()){
                ...
                String nick = cursor.getString(cursor.getColumnIndex(InviteMessgeDao.COLUMN_USER_NICK));
                String userAvatar = cursor.getString(cursor.getColumnIndex(InviteMessgeDao.COLUMN_USER_AVATAR));
